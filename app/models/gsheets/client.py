...
class GSheetsClient:
    ...
    async def get_or_create_sheet(
        self, group_number: int, group: Group, journal: Journal
    ) -> str:
        ...
        if cached := await self._redis.get(cache_key):
            try:
                spreadsheet = await self._open(cached.decode())
            except Exception:
        ...
        await self._filler.add_dated_sheet(spreadsheet, group, journal)
    ...
    async def update_all_sheets(
        ...
        for row in rows:
            journal = journals.get(row.group_number)
            group = groups_map.get(row.group_number)
            if not journal or not group:
                continue
            try:
                spreadsheet = await self._open(row.spreadsheet_id)
                await self._filler.add_dated_sheet(spreadsheet, group, journal)
                await self._logger.ainfo("sheet_updated", group_number=row.group_number)
